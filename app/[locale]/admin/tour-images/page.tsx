...
export default function AdminTourImagesPage() {
    ...
    try {
      ...
      if (fetchError) {
        ...
        if (fetchError.message?.includes('does not exist') || 
          ...
          console.error('Tour images column does not exist. Please run migration to add tour_images column to tours_table_prod.');
          setTableExists(false);
          setError('Database migration required. Please add tour_images JSONB column to tours_table_prod table.');
          return {};
        }
        console.error('Failed to fetch tour images:', fetchError);
        return {};
      }
  ...
  const loadData = async () => {
    ...
    try {
      ...
      setAllTourImages(imagesData);

      // Merge tours with images
      const toursWithImages: TourWithImages[] = (toursData || []).map((tour) => {
        const images = imagesData[tour.id] || [];
  ...
  const openDialog = async (tour: Tour) => {
    setSelectedTour(tour);
    const images = allTourImages[tour.id] || [];
    ...
    if (firstImage?.storage_folder_name) {
      ...
      const matchedFolder = await matchFolderToTour(tour);
      if (matchedFolder) {
  ...
  const matchFolderToTour = async (tour: Tour): Promise<string | null> => {
    // Try exact ID match
    const idMatch = availableFolders.find((folder) => folder === tour.id);
    ...
    const normalizedTitle = tour.title.toLowerCase()
      .replace(/[^a-z0-9]+/g, '-')
      .replace(/^-+|-+$/g, '');
    const titleMatch = availableFolders.find(
      (folder) => folder.toLowerCase() === normalizedTitle
    );
    if (titleMatch) return titleMatch;

    // Try partial match (folder contains ID or title)
    const partialMatch = availableFolders.find(
      (folder) => folder.includes(tour.id) || folder.toLowerCase().includes(normalizedTitle)
    );
  ...
  const handleSaveImages = async () => {
    ...
    if (!selectedFolder) {
      toast.error('Please select or create a folder for this tour');
      return;
    }
    ...
    try {
      ...
      // Update tours_table_prod with new images array
      const { error: updateError } = await supabase
        .from('tours_table_prod')
        .update({ tour_images: allImages })
        .eq('id', selectedTour.id);
      ...
      if (updateError) {
        if (updateError.message?.includes('does not exist') || 
          ...
          toast.error('Database migration required. Please add tour_images JSONB column to tours_table_prod table.');
          setTableExists(false);
          return;
        }
        throw updateError;
  ...
  const handleSetPrimary = async (imageId: string) => {
    ...
    try {
      ...
      const updatedImages = tourImages.map(img => ({
        id: img.id,
        url: img.image_url,
      ...
      const { error } = await supabase
        .from('tours_table_prod')
        .update({ tour_images: updatedImages })
        .eq('id', selectedTour.id);
  ...
  const handleDeleteImage = async (imageId: string, imageUrl: string) => {
    ...
    try {
      ...
      const updatedImages = tourImages.filter(img => img.id !== imageId);
      ...
      const { error: dbError } = await supabase
        .from('tours_table_prod')
        .update({ tour_images: imagesForDb })
        .eq('id', selectedTour.id);
  ...
  const handleReorder = async (imageId: string, direction: 'up' | 'down') => {
    const currentIndex = tourImages.findIndex((img) => img.id === imageId);
    if (currentIndex === -1) return;

    const newIndex = direction === 'up' ? currentIndex - 1 : currentIndex + 1;
    if (newIndex < 0 || newIndex >= tourImages.length) return;
    ...
    try {
      ...
      const { error } = await supabase
        .from('tours_table_prod')
        .update({ tour_images: reorderedImages })
        .eq('id', selectedTour?.id);
  ...
  const handleCreateFolder = async () => {
    ...
    try {
      ...
      if (created) {
        ...
        // Also link it to the tour if one is selected
        if (selectedTour) {
  ...
  const handleLinkFolder = async () => {
    if (!selectedTour || !selectedFolder) return;
    ...
    try {
      ...
      // Update all images for this tour with the folder name
      const updatedImages = tourImages.map(img => ({
        id: img.id,
        url: img.image_url,
        is_primary: img.is_primary,
      ...
      toast.success('Folder linked to tour');
      await loadData();
    } catch (err: any) {
  ...
  const handleAddSelectedFolderImages = async () => {
    if (!selectedTour || !selectedFolder || selectedFolderImages.length === 0) return;
    ...
    try {
      ...
      const maxSortOrder = tourImages.length > 0
        ? Math.max(...tourImages.map((img) => img.sort_order))
        : -1;
      ...
      const existingImages = tourImages.map(img => ({
        id: img.id,
        url: img.image_url,
        is_primary: img.is_primary,
        sort_order: img.sort_order,
      ...
      toast.success(`Added ${newImages.length} image(s) to tour`);
      setSelectedFolderImages([]);
    ...
    <div className="min-h-screen bg-gray-50 p-4 md:p-8">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mb-6 flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Tour Images Management</h1>
            <p className="mt-1 text-sm text-gray-500">Manage images for tours</p>
          </div>
        ...
        {loading ? (
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {filteredTours.map((tour) => {
              const primaryImageUrl = tour.primaryImage?.image_url;
              ...
              return (
                <Card key={tour.id} className="overflow-hidden">
                  <CardHeader>
                    ...
                    <CardDescription>
                      {tour.city} • {tour.type} • {tour.duration_minutes}min
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
        ...
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          <DialogContent className="max-h-[90vh] max-w-4xl overflow-y-auto">
            <DialogHeader>
              <DialogTitle>Manage Images for {selectedTour?.title}</DialogTitle>
              <DialogDescription>
                Upload, set primary, and reorder images for this tour
              </DialogDescription>
            </DialogHeader>
                      ...
                      >
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          <Button
                            ...
                            disabled={index === tourImages.length - 1}
                          >
